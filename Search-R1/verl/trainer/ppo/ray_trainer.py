...
def apply_kl_penalty(data: DataProto, kl_ctrl: core_algos.AdaptiveKLController, kl_penalty='kl'):
    """
    Apply KL penalty to token-level scores to prevent the policy from deviating too much from the reference policy.
    
    The KL penalty serves several important purposes:
    1. **Policy Regularization**: Prevents the policy from diverging too far from the reference policy during training,
       which helps maintain stability and prevents mode collapse.
    2. **Exploration vs Exploitation Balance**: Controls the trade-off between exploiting high-reward behaviors 
       and maintaining diversity in the policy's outputs.
    3. **Training Stability**: Reduces the risk of catastrophic forgetting and helps maintain coherent language 
       generation capabilities from the original pre-trained model.
    4. **Reward Hacking Prevention**: Prevents the model from finding adversarial ways to maximize rewards 
       that don't align with the intended behavior.
    
    The final reward is computed as: reward = original_score - beta * KL_divergence
    where beta is the KL coefficient that controls the strength of the penalty.
    
    Args:
        data: DataProto containing batch data with responses, scores, and log probabilities
        kl_ctrl: Adaptive KL controller that manages the KL coefficient dynamically
        kl_penalty: Type of KL penalty to apply (default: 'kl')
    
    Returns:
        tuple: (modified data with token_level_rewards, metrics dict)
    """
    ...
    if 'ref_log_prob' in data.batch.keys():
        # log_probs represent the log probabilities of tokens generated by the policy model
        # - old_log_probs: log probabilities from the current policy before this update step
        # - ref_log_prob: log probabilities from the reference (original) policy model
        # This differs from the value function which estimates expected future rewards (V(s))
        # while log_probs measure the likelihood of generating specific tokens given the context
